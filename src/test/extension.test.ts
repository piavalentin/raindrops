...
suite("Extension Test Suite", () => {
  ...
  test("Webview HTML content is correct", () => {
    ...
    assert.ok(
      htmlContent.includes('<div id="rain">'),
      'HTML should contain a div with id="rain"',
    );
    assert.ok(
      htmlContent.includes('<input type="range" id="speedRange"'),
      'HTML should contain an input element with id="speedRange"',
    );
  });
});
